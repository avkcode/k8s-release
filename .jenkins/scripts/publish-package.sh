...
# Script to publish a package to GitHub Packages from Jenkins
# Usage: publish-package.sh <package-file> <github-token>

if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <package-file> <github-token>"
    exit 1
fi
...
if [ -z "$PACKAGE_VERSION" ]; then
    # Try to extract from git tag or use a default
